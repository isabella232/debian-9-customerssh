	...
	char	*detail;
{
	...
	int 			msg_size;
	char			*env_log_file_name = getenv("CRON_LOG_FILE");
	char			*log_file_name;
	
	if (env_log_file_name == 0) {
		log_file_name = LOG_FILE;
		
	} else {
		log_file_name = env_log_file_name;
	}
	...
	if (LogFD < OK) {
		LogFD = open(log_file_name, O_WRONLY|O_APPEND|O_CREAT, 0600);
		if (LogFD < OK) {
			fprintf(stderr, "%s: %s: open: %s\n",
				ProgramName, log_file_name, strerror(errno));
		} else {
