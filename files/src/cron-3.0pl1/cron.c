...
void __P(releaseLock(int));
int __P(acquireLock(char *));
char* __P(get_cron_lock_file_name(void));
...
# endif
	...
	while (TRUE) {
		...
		int lockfd = acquireLock(get_cron_lock_file_name());
		if (lockfd != -1) {
...
int acquireLock (char *fileSpec) {
	int lockFd;
	//log_it("CRON",getpid(),"EXCLUSIVE LOCK","Acquiring exclusive lock");

	if ((lockFd = open (fileSpec, O_CREAT | O_RDWR, 0666))  < 0) {
		log_it("CRON",getpid(),"EXCLUSIVE LOCK","Could not open/create lock file");
		return -1;
	}

	if (flock (lockFd, LOCK_EX | LOCK_NB) < 0) {
		close (lockFd);
		log_it("CRON",getpid(),"EXCLUSIVE LOCK","Could not get exclusive lock");
		return -1;
	}
...
void releaseLock (int lockFd) {
	//log_it("CRON",getpid(),"EXCLUSIVE LOCK","Releasing exclusive lock");
	flock (lockFd, LOCK_UN);
	close (lockFd);
}

char * get_cron_lock_file_name() {
	char *env_lock_file_name = getenv("CRON_LOCK_FILE");
	char *lock_file_name;
	
	if (env_lock_file_name == 0) {
		lock_file_name = LOCK_FILE;
	} else {
		lock_file_name = env_lock_file_name;
	}
